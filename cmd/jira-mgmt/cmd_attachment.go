...
func writePrivateFile(path string, data []byte) error {
	...
	// Publish with create-if-absent semantics. Rename would overwrite a file
	// created after the Lstat check, while a hard link fails atomically when the
	// destination already exists. The temporary file lives in the same
	// directory, so both names are necessarily on the same filesystem.
